...
// Constantes mensagens de gol
const MENSAGEM_DEFAULT = "🎉 TOCA A MÚSICAAAAA 🎉";
const LISTA_MENSAGENS_DE_GOL = [MENSAGEM_DEFAULT, "🎊 TÁ COM ALEGRIA NAS PERNA 🎊", "XOXOTOU A CARA DO GOLEIRO"];

// Variáveis de comportamento
let SHOW_STATS_ON_VICTORY = false;
let HOSTING_ON_MATRIX = true;
...
// Variáveis da sala
let playersThatTouchedTheBall = new Set();
...
room.onGameStart = function(byPlayer)
{
    ...
    teamStatsController.restartMathStats();
...
room.onTeamGoal = function(teamID) {
    ...
    let iTplayersThatTouchedTheBall = playersThatTouchedTheBall.values();
    
    let striker = room.getPlayer(iTplayersThatTouchedTheBall.next().value);

    let assistant = iTplayersThatTouchedTheBall.next().value;
...
function handleBallTouch() {
	...
	players_playing.forEach((player) => {
        ...
        if ( distanceToBall < triggerDistance ) {
            if (!playersThatTouchedTheBall.has(player.id))
            {
