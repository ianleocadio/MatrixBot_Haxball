...
const room = HBInit({
    ...
    public: true,
    token: "thr1.AAAAAF5Pz7LhT2OFbhNQ2Q.W62fteZlUjE"
});
...
const CommandController = class CommandController
{
    ...
    showGolsList() {
        ...
        {
            listDeGols.forEach((obj) =>
            {
                texto += obj.name + " - Gols: " + obj.Gols + "\n" ;
            });
        }
...
room.onTeamGoal = function (teamID) {
    ...
    // Incrementa gol na lista
    incrementaGol(lastPlayerKicked);
}
...
var incrementaGol = (player) =>
{
    console.log(player);
    // Lista vázia
    if (listDeGols.length == 0) { listDeGols.push({"id": player.id, "name": player.name, "Gols": 1}); return; }

    // Lista de gols já existente
    var playerInList = listDeGols.find((o) => o.id == player.id);
    // Lista de gols existente, mas jogador n tinha feito gol ainda
    if (playerInList == null) { listDeGols.push({"id": player.id, "name": player.name, "Gols": 1}); return; }

    // Jogador já tinha feito gol, incrementando valor
    playerInList.name = player.name;
    playerInList.Gols += 1;
}
